    ...
    <div class="avatar avatar">
                                        ...
                                        <div class="message-inner">
                                            ...
                                            <div class="message-footer">
                                                <span
                                                    class="extra-small text-muted">${moment(
                                                        msg.created_at
                                                    ).fromNow()}</span>
                                            </div>
                                        </div>
...
const conversation = function (chat) {
    console.log(chat)
    $('#chat-list').append(`<a href="#${chat.id}" data-messages="${chat.id}"
                                                ...
                                                <div class="row gx-5">
                                                    ...
                                                    <div class="col">
                                                        <div class="d-flex align-items-center mb-3">
                                                            ...
                                                            <span
                                                                class="text-muted extra-small ms-2">${moment(
                                                                    chat
                                                                        .last_message
                                                                        .created_at
                                                                ).fromNow()}</span>
...
$('#chat-list').on('click', '[data-messages]', function (e) {
    ...
    $.get(`/api/conversations/${id}/messages`, function (response) {
        ...
        $('#chat-avatar').text(response.conversation.participants[0].name[0]);
        console.log(response);
        for (i in response.messages.data) {
            let msg = response.messages.data[i];
            let c = msg.user_id == id ? 'message-out' : '';
            addMessage(msg, c)
        }
    })
})
